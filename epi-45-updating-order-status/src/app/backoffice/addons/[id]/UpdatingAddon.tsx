...
interface Props {
  addonToBeUpdatedOrDeleted: Addons;
  addonCategories: AddonCategories[] | null;
  id: number;
}

export default function UpdatingAddonPage({
  addonToBeUpdatedOrDeleted,
  addonCategories,
  id,
}: Props) {
  const router = useRouter();

  const handleUpdatingAddon = async (formData: FormData) => {
    const response = await updatingAddon(formData);
    console.log("response is", response);
    if (response.error) {
      toast.error(response.error);
    } else {
      toast.success("Addon is updated successfully");
      router.push("/backoffice/addons");
    }
  };

  const handleDeleteAddon = async (formData: FormData) => {
    const response = await deleteAddon(formData);
    console.log("response is", response);
    if (response.error) {
      toast.error(response.error);
    } else {
